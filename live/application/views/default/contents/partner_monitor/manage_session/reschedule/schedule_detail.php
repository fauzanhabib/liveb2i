...
<script type="text/javascript">
    $(function () {
        ...
        $(".datepicker").on('change', function() {
            //alert(this.name);
            var loadUrl = "<?php echo site_url('partner_monitor/manage_session/reschedule_session/'.$appointment_id.'/'.$coach_id); ?>"+ "/" +$(this).val();
            //alert(loadUrl);
            if($(this).val() != ''){
        ...
        $(".weekly_schedule").click(function () {
            //alert(this.name);
            var loadUrl = "<?php echo site_url('partner_monitor/schedule/schedule_detail'); ?>" + "/" + this.value;
            //alert(loadUrl);
            if (this.value != '') {
                $("#schedule-loading").show();
                $("#confirm_button").hide();
        ...
        $(".confirm_button").click(function(){
            ...
            window.location = '<?php echo site_url('partner_monitor/manage_session/reschedule_booking/'.$student_id.'/'.$appointment_id.'/'.$coach_id.'/')?>'+'/'+$(".datepicker").val()+'/'+myarr[0]+'/'+myarr[1];
        });

        $(".cancel_button").click(function(){
            window.location = '<?php echo site_url('partner_monitor/schedule/manage/'.$student_id)?>';
        });
    });
