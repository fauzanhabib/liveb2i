...
<script>
    document.getElementById("submit_summary").onclick = function () {
        location.href = "<?php echo site_url('partner_monitor/schedule/book_single_coach/' . $data_student[0]->id .'/'. $data_coach[0]->id . '/' . $date . '/' . $start_time . '/' . $end_time); ?>";
    };

    document.getElementById("cancel_summary").onclick = function () {
        location.href = "<?php echo site_url('partner_monitor/schedule/create/' . $search_by) ; ?>";
    };
</script>