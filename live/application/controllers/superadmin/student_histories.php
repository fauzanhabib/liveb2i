...
class Student_histories extends MY_Site_Controller {
    ...
    //     $per_page = 5;
    //     $uri_segment = 5;
    //     $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_histories/index/'.@$student_id), count($this->appointment_model->get_session_histories('student_id', $student_id, $date_from, date('Y-m-d'))), $per_page, $uri_segment);
    ...
    public function index($student_id='', $page=''){
        ...
        $per_page = 5;
        $uri_segment = 5;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('superadmin/student_histories/index/'.@$student_id), count($this->appointment_model->get_session_histories('student_id', $student_id, $date_from, date('Y-m-d'))), $per_page, $uri_segment);
        $histories = $this->appointment_model->get_session_histories('student_id', $student_id, $date_from, date('Y-m-d'), $per_page, $offset);
        if ($histories) {
                ...
                //     }
                    ...
                    if(@!$gmt_student){
                    $data_schedule = $this->schedule_function->convert_book_schedule($this->identity_model->new_get_gmt($adm_id)[0]->minutes, strtotime($history->date), $history->start_time, $history->end_time);
                    }else{
    ...
    //             redirect('admin/student_histories/index/'.@$student_id);
    //         }elseif($session == 'class'){
    //             redirect('admin/student_histories/class_session/'.@$student_id);
    //         }
    //     }
    ...
    //         $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_histories/search/one_to_one/'.@$student_id), count($this->appointment_model->get_session_histories('student_id', @$student_id, $date_from, $date_to, $per_page, $offset)), $per_page, $uri_segment);
    ...
    //         $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_histories/search/class/'.@$student_id), count($this->class_member_model->get_session_histories('student_id', @$student_id, $date_from, $date_to, $per_page, $offset)), $per_page, $uri_segment);
    ...
    public function search($session='', $student_id='', $page=''){
        ...
        if($session == 'one_to_one'){
            $form_action = "search/one_to_one";
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('superadmin/student_histories/search/one_to_one/'.@$student_id), count($this->appointment_model->get_session_histories('student_id', @$student_id, $date_from, $date_to, $per_page, $offset)), $per_page, $uri_segment);
            $histories = $this->appointment_model->get_session_histories('student_id', @$student_id, $date_from, $date_to, $per_page, $offset);
            if ($histories) {
                foreach ($histories as $h) {
                    $gmt_student = $this->identity_model->new_get_gmt($student_id);
    ...
    public function class_session($student_id='', $page=''){
        ...
        $per_page = 5;
        $uri_segment = 5;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_histories/class_session/'.@$student_id), count($this->class_member_model->get_session_histories('student_id', $student_id, $date_from, date('Y-m-d'))), $per_page, $uri_segment);
