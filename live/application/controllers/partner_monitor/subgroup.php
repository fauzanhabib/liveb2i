...
class subgroup extends MY_Site_Controller {
  ...
  //       $per_page = 6;
  //       $uri_segment = 4;
		// $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('partner_monitor/member_list/coach'.$subgroup_id), count($this->identity_model->get_coach_identity('','',$this->auth_manager->partner_id(),$subgroup_id)), $per_page, $uri_segment);
  ...
  //       $this->template->content->view('default/contents/partner_monitor/managing_subgroup/index', $vars);
  ...
  //   }
    ...
    public function index_disable($page='') {
        ...
        $this->template->content->view('default/contents/partner_monitor/managing_subgroup/index_disable', $vars);
    ...
    public function add_subgroup() {
        ...
        $this->template->content->view('default/contents/partner_monitor/managing_subgroup/add_subgroup_form', $vars);
        $this->template->publish();
    }
    ...
    public function create_subgroup() {
        ...
        if (!$this->input->post('__submit')) {
            ...
            redirect('partner_monitor/subgroup');
        }
        if(!$partner_id){
        ...
        $rules = array(
            ...
            if (!$this->common_function->run_validation($rules)) {
                ...
                redirect('partner_monitor/subgroup/add_subgroup');
            }
    ...
    { 
        ...
        $this->template->content->view('default/contents/partner_monitor/setting', $vars);
        $this->template->publish();
    }
    ...
    //---------------------------------------------------------------------------------------------------------------------
     public function edit_subgroup($subgroup_id = '', $page='', $id='') {
        ...
        $per_page = 6;
        $uri_segment = 4;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('partner_monitor/managing_subgroup/detail'.$subgroup_id), count($this->identity_model->get_coach_identity('','','',$this->auth_manager->partner_id(),$subgroup_id)), $per_page, $uri_segment);
        ...
        $this->template->content->view('default/contents/partner_monitor/managing_subgroup/detail', $vars);
        $this->template->publish();
    }
    ...
    public function list_disable_coach($subgroup_id = '', $page='', $id='') {
        ...
        $this->template->content->view('default/contents/partner_monitor/managing_subgroup/detail_disable', $vars);
        $this->template->publish();
    }
    ...
    function delete_coach($id){
        if(!empty($_POST['check_list'])) {
            ...
            if($check_appointment){
                ...
                redirect('partner_monitor/subgroup/list_coach/'.$id);
    ...
    public function create_coach() {
        ...
        if (!$this->input->post('__submit')) {
            $this->messages->add('Invalid action', 'danger');
            redirect('partner_monitor/subgroup/coach');
        }
...
//        print_r($user_id); exit;
    ...
    public function coach_detail($subgroup_id = '', $id = ''){
        ...
        if(!$data){
            $this->messages->add('Invalid Action', 'warning');
            redirect('partner_monitor/subgroup/list_coach/'.$subgroup_id);
        }
        ...
        $this->template->content->view('default/contents/partner_monitor/managing_subgroup/coach_detail', $vars);
        $this->template->publish();
    }
