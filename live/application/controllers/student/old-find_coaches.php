...
//        print_r($vars); exit;
                        ...
                        for ($i = 0; $i < $duration; $i++) {
                            ...
                            if(strtotime($availability_exist['end_time']) <= strtotime($a['end_time'])){
                                ...
                                if (in_array($availability_exist['start_time'], $appointment_start_time_temp) && in_array($availability_exist['end_time'], $appointment_end_time_temp)) {
                                    ...
                                    if($this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes > 0){
                                        date_default_timezone_set('Etc/GMT'.($this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes/60 >= 0 ? '+'.$this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes/60 : $this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes/60));
                                    }
                                }
                            }
    ...
    public function book_single_coach($coach_id = '', $date_ = '', $start_time_ = '', $end_time_ = '') {
            ...
            if ($isValid) {
                ...
                if ($this->db->trans_status() === true && $remain_token >= 0 && $this->isAvailable($coach_id, $date, $start_time, $end_time)) {
                    ...
                    if ($this->db->trans_status() === true && $appointment_id && $valid_appointment == 1) {
                        ...
                        $message = 'Book successed';

                        $this->send_email->student_book_coach($emailstudent[0]->email, $emailcoach[0]->email, $namestudent[0]->fullname, $namecoach[0]->fullname, $start_time, $end_time, $dateconvert, 'booked');
                        $this->send_email->notif_coach($emailstudent[0]->email, $emailcoach[0]->email, $namestudent[0]->fullname, $namecoach[0]->fullname, $start_time, $end_time, $dateconvert, 'booked');
                        
                 
                         $member_notification = array(
                            'user_id' => $coach_id,
                            'description' => 'You have a new booked from your student',
                            'status' => 2,
                            'dcrea' => time(),
                            'dupd' => time(),
                        );
    ...
    public function book_by_multiple_date_index($index = '') {
        ...
        //echo('<pre>');print_r($this->session->userdata('date_1'));exit;
        $vars = array(
            'data' => $data,
            'temporary_booking' => $this->appointment_model->where('student_id', $this->auth_manager->userid())->where('status', 'temporary')->get_all(),
...
//                        break;
    ...
    public function availability($search_by = '', $coach_id = '', $date_ = '') {
        ...
        //to make static date function before the data being converted
...
//                exit;
                if (in_array(array('start_time' => $start_time_available, 'end_time' => $end_time_available), $availability)) {
                    ...
                    $this->messages->add('Invalid Time2', 'warning');
                    redirect('student/find_coaches/search/name/');
                }
                // begin the transaction to ensure all data created or modified structural
                $token_cost = $this->coach_token_cost_model->select('token_for_student')->where('coach_id', $coach_id)->get();
    ...
    public function max_book_coach_per_day($coach_id='',$date=''){
        ...
        $get_max_per_day = $this->db->select('max_session_per_day, max_day_per_week')->from('specific_settings')->where('partner_id',$partner_id)->get()->result();
        $max_per_day = $get_max_per_day[0]->max_session_per_day;
...
//        print_r($schedule); exit;
        ...
        if ($appointment || $appointment_student || $appointment_class) {
            ...
            if($appointment_count < $setting[0]->max_session_per_day && $appointment_count_week < $setting[0]->max_day_per_week){
