...
class manage_partner extends MY_Site_Controller {
    ...
    public function index($page='') {
        ...
        $per_page = 6;
        $uri_segment = 4;
        // $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin_m/manage_partner/index'), count($this->partner_model->select('id, profile_picture, name, address, country, state, city, zip')->where('name not like', 'No Partner')->where('admin_regional_id',$id_region)->order_by('name', 'asc')->get_all()), $per_page, $uri_segment);
        
        // search
        $search_region = $this->input->post('search_partner');
        if($search_region != ''){
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin_m/manage_partner/index'), count($this->partner_model->select('id, profile_picture, name, address, country, state, city, zip')->where('name not like', 'No Partner')->like('name',$search_region)->where('admin_regional_id',$id)->order_by('id', 'desc')->get_all()), $per_page, $uri_segment);           
            ...
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin_m/manage_partner/index'), count($this->partner_model->select('id, profile_picture, name, address, country, state, city, zip')->where('name not like', 'No Partner')->where('admin_regional_id',$id)->order_by('id', 'desc')->get_all()), $per_page, $uri_segment);
    ...
    public function create_partner() {
        ...
        $rules = array(
            ...
            if (!$this->common_function->run_validation($rules)) {
                ...
                redirect('admin_m/manage_partner/add_partner');
            }
    ...
    public function update_partner($id = '') {
        ...
        redirect('admin_m/manage_partner/detail/'.$id);
    }
...
$id = $pull_m_id[0]->id;
            ...
            if($user_token < $request_token){
                ...
                redirect('admin_m/manage_partner/add_partner_member/'.$partner_id.'/'.$this->input->post('partner_id').'/student/');
            }
        ...
        if($this->input->post('role_id') == 3){
            redirect('admin_m/manage_partner/supplier/coach/'.$this->input->post('partner_id'));
        }elseif($this->input->post('role_id') == 5){
            redirect('admin_m/manage_partner/supplier/student/'.$this->input->post('partner_id'));
        }
    }
    ...
    public function subgroup_action($type = '',$partner_id = '') {
        ...
        if(!empty($_POST['check_list'])) {
            ...
            redirect('admin_m/manage_partner/list_supplier/'.$type.'/'.$partner_id);
    }
    ...
    public function coach_detail($partner_id='', $coach_id=''){
        ...
        if(!$data){
            ...
            redirect('admin_m/manage_partner/'.$partner_id);
        }
        $vars = array(
            'coach_id' => $coach_id,
            'data' => $data,
    ...
    //     $per_page = 6;
    //     $uri_segment = 6;
    //     $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin_m/manage_partner/member_of_student/'.$status.'/'.$subgroup_id.'/'.$partner_id), count($this->user_profile_model->get_students($partner_id,$subgroup_id,$new_status)), $per_page, $uri_segment);
    //     $vars = array(
    ...
    //     $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin_m/manage_partner/member_of_coach/'.$status.'/'.$subgroup_id.'/'.$partner_id), count($this->user_profile_model->get_coaches($partner_id,$subgroup_id,$new_status)), $per_page, $uri_segment);
    //     $vars = array(
    //         'subgroup_id' => $subgroup_id,
    //         'partner' => $partner,
    //         'partner_id' => $partner_id,
    ...
    function delete_student($subgroup_id = '',$partner_id = ''){
        ...
        if(!empty($_POST['check_list'])) {
            ...
            redirect('admin_m/manage_partner/member_of_student/active/'.$subgroup_id.'/'.$partner_id );
    }
    ...
    function delete_coach($subgroup_id = '',$partner_id = ''){
        ...
        if(!empty($_POST['check_list'])) {
            ...
            redirect('admin_m/manage_partner/member_of_coach/active/'.$subgroup_id.'/'.$partner_id );
    }
    ...
    public function coach_action($type='',$subgroup_id='',$partner_id=''){
        ...
        if(!empty($_POST['check_list'])) {
            ...
            redirect('admin_m/manage_partner/member_of_coach/'.$type.'/'.$subgroup_id.'/'.$partner_id);
    ...
    public function student_action($type='',$subgroup_id='',$partner_id=''){
        ...
        if(!empty($_POST['check_list'])) {
            ...
            redirect('admin_m/manage_partner/member_of_student/'.$type.'/'.$subgroup_id.'/'.$partner_id);
    ...
    public function upload_profile_picture($partner_id=''){
        ...
        redirect('admin_m/manage_partner/detail/'.$partner_id);
    }
    ...
    public function approve_token($token_request_id = '') {
        ...
        if ($this->token_request_model->get_token_request($token_request_id,5)) {
            ...
            if($get_token_region->token_amount < $token_request->token_amount){
                ...
                redirect('admin_m/manage_partner/token');
            }
    ...
    function update_setting($id){
       ...
       // =============================
            ...
            if (!$this->common_function->run_validation($rules)) {
                $this->messages->add(validation_errors(), 'warning');
                redirect('admin_m/manage_partner/setting/'.$id.'/student');
            }
       ...
       redirect('admin_m/manage_partner/setting/'.$id.'/'.$type);
    }
    ...
    function add_token($student_supplier_id='',$partner_id=''){
        ...
        $link = base_url().'index.php/admin_m/manage_partner/action_add_token/'.$student_supplier_id.'/'.$partner_id;
        $cancel = base_url().'index.php/admin_m/manage_partner/partner/student/'.$partner_id;
        $vars = array(
                'link' => $link,
                'cancel' => $cancel,
                'token' => $token
...
$id = $pull_m_id[0]->id;
        ...
        if (!$this->input->post('token') || $this->input->post('token') <=0) {
            ...
            redirect('admin_m/manage_partner/add_token/'.$student_supplier_id.'/'.$partner_id);
        }
        ...
        redirect('admin_m/manage_partner/partner/student/'.$partner_id);
