...
class token extends MY_Site_Controller {
    ...
    public function index() {
        ...
        $this->template->content->view('default/contents/admin_m/token/index', $vars, $overwrite=false);
    ...
    public function add() {
        if ($this->isRequested()) {
            ...
            redirect('admin_m/request_token');
        }
        ...
        $this->template->content->view('default/contents/admin_m/token/form', $vars, $overwrite=false);
        $this->template->publish();
    }
    ...
    function history($page=''){
        ...
        $per_page = 10;
        $uri_segment = 4;
        // $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin_m/token/history/'), count($this->history_request_model->select('token_requests.id, up.region_id as region_id, u.email, up.fullname, token_requests.token_amount, token_requests.status')->join('users u', 'u.id = token_requests.user_id')->join('user_profiles up', 'up.user_id = u.id')->where('u.role_id', 4)->where('token_requests.user_id',$this->auth_manager->userid())->where('u.status', 'active')->where('token_requests.status !=','requested')->order_by('up.fullname', 'asc')->get_all()), $per_page, $uri_segment);
...
$id = $pull_m_id[0]->id;
        ...
        $this->template->content->view('default/contents/admin_m/token/history', $vars);
