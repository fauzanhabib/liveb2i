...
class subgroup extends MY_Site_Controller {
    ...
    public function __construct() {
        ...
        if (!$this->auth_manager->role() || $this->auth_manager->role() != 'SAM') {
    ...
    public function index($page='') {
        ...
        if($search_subgroup != ''){
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/index'), count($this->identity_model->get_subgroup_identity(null,'student',null, $search_subgroup)), 9999, $uri_segment);
            ...
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/index'), count($this->identity_model->get_subgroup_identity(null,'student',null,null)), $per_page, $uri_segment);
        ...
        $this->template->content->view('default/contents/student_aff_m/managing_subgroup/index', $vars);
    ...
    public function index_disable($page='') {
        ...
        if($search_subgroup != ''){
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/index_disable'), count($this->identity_model->get_subgroup_identity(null,'student',null, $search_subgroup)), 9999, $uri_segment);
            $data = $this->identity_model->get_subgroup_identity(null,'student','disable',null,$search_subgroup,9999, $offset);
            $data2 = $this->identity_model->get_student_identity('','',$this->auth_manager->partner_id(), '', 9999, $offset, '');
        } else if($search_subgroup == ''){
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/index_disable'), count($this->identity_model->get_subgroup_identity(null,'student',null,null)), $per_page, $uri_segment);
        ...
        $this->template->content->view('default/contents/student_aff_m/managing_subgroup/index_disable', $vars);
    ...
    public function delete_subgroup($id=''){
            ...
            redirect ('student_aff_m/subgroup');
    ...
    public function add_subgroup() {
        ...
        $this->template->content->view('default/contents/student_aff_m/managing_subgroup/add_subgroup_form', $vars);
        $this->template->publish();
    }
    ...
    public function create_subgroup() {
        ...
        $rules = array(
            ...
            if (!$this->common_function->run_validation($rules)) {
                ...
                redirect('student_aff_m/subgroup/add_subgroup');
            }
    ...
    //---------------------------------------------------------------------------------------------------------------------
     ...
     public function list_student($subgroup_id = '', $page='') {
        ...
        if($search_student != ''){
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/list_student/'.$subgroup_id), count($this->identity_model->get_student_identity('',$search_student,$this->auth_manager->partner_id(),'', '', '', $subgroup_id)), 9999, $uri_segment);
            ...
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/list_student/'.$subgroup_id), count($this->identity_model->get_student_identity('','',$this->auth_manager->partner_id(),'', '', '', $subgroup_id)), $per_page, $uri_segment);
    ...
    public function list_disable_student($subgroup_id = '', $partner_id = '', $page='') {
        ...
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/subgroup/list_disable_student/'.$partner_id), count($this->identity_model->get_student_identity('','',$this->auth_manager->partner_id(),'')), $per_page, $uri_segment);
        // echo $subgroup_id." - ".$id." -". $page." = ".$per_page;exit();
        // echo "id ".$id;
        ...
        $this->template->content->view('default/contents/student_aff_m/managing_subgroup/detail_disable', $vars);
        $this->template->publish();
    }
    ...
    public function edit_subgroup($subgroup_id = '', $page='', $id='') {
        ...
        $per_page = 6;
        $uri_segment = 4;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_aff_m/member_list/student'.$subgroup_id), count($this->identity_model->get_student_identity('','',$this->auth_manager->partner_id(),$subgroup_id)), $per_page, $uri_segment);
    ...
    function delete_student($subgroup_id = ''){
        ...
        if(!empty($_POST['check_list'])) {
            ...
            if(in_array("disable", $list_stat) && !in_array("active", $list_stat)){
              ...
              redirect('student_aff_m/subgroup/list_student/'.$subgroup_id);
            }
    ...
    public function setting($subgroup_id='', $id='')
    {
        ...
        $this->template->content->view('default/contents/student_aff_m/setting', $vars);
        $this->template->publish();
    }
    ...
    function update_setting($id, $subgroup_id){
        ...
        if (!$this->common_function->run_validation($rules)) {
            $this->messages->add(validation_errors(), 'warning');
            redirect('student_aff_m/subgroup/setting/'.$id);
        }
    ...
    public function create_student() {
        ...
        if (!$this->input->post('__submit')) {
            $this->messages->add('Invalid action', 'warning');
            redirect('student_aff_m/member_list/student');
        }
        ...
        if(@$student_member >= @$setting->max_student_class){
            ...
            redirect('student_aff_m/subgroup/student');
        }
