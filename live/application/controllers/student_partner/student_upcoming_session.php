...
class Student_upcoming_session extends MY_Site_Controller {
    ...
    //     $per_page = 5;
    //     $uri_segment = 5;
    //     $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_partner/student_upcoming_session/one_to_one_session/'.$student_id), count($this->appointment_model->get_appointment_for_upcoming_session('student_id', '', '', $student_id)), $per_page, $uri_segment);
    ...
    //     $this->template->content->view('default/contents/student_partner/student_upcoming_session/one_to_one_session', $vars);
    //     $this->template->publish();
    // }
    ...
    public function one_to_one_session($student_id='', $page='') {
        ...
        if ($data) {
            foreach ($data as $d) {
                // $gmt_coach = $this->identity_model->new_get_gmt($d->coach_id);
                //     if(@!$gmt_coach){
                ...
                //     }else{
                //     $data_schedule = $this->schedule_function->convert_book_schedule($this->identity_model->new_get_gmt($d->coach_id)[0]->minutes, strtotime($d->date), $d->start_time, $d->end_time);
                //     }
    ...
    public function class_session($student_id='', $page='') {
        ...
        $per_page = 5;
        $uri_segment = 5;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_partner/student_upcoming_session/class_session/'.@$student_id), count($this->class_member_model->get_appointment_for_upcoming_session('', '', @$student_id)), $per_page, $uri_segment);
        ...
        $this->template->content->view('default/contents/student_partner/student_upcoming_session/class_session', $vars);
        $this->template->publish();
    }
    ...
    //         $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('student_partner/student_upcoming_session/search/class/'.@$student_id), count($this->class_member_model->get_appointment_for_upcoming_session($date_from, $date_to, $student_id)), $per_page, $uri_segment);
