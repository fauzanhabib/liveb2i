...
class Add_token extends MY_Site_Controller {
    ...
    public function group_index($page='', $subgroup_id = '') {
        ...
        $this->template->content->view('default/contents/student_partner/reporting/group_index', $vars);
        $this->template->publish();
    }
    ...
    function action_add_token(){
        ...
        $get_name2 = $this->user_profile_model->select('user_id, fullname, phone')->where('user_id', $student_id)->get_all();
        ...
        // $b = $this->session->flashdata('results');
        // if($b == 'DELIVERED_TO_HANDSET'){
        //     echo "ok";
        //     exit();
        // }else{
        //     echo "nah";
    ...
    function action_add_group_token(){
        date_default_timezone_set('Etc/GMT+0');
        ...
        $total_student = $this->db->select('user_profiles.fullname, user_tokens.token_amount, users.id as id')
                                ->from('user_profiles')
                                ->join('users','users.id = user_profiles.user_id')
                                ->join('subgroup','user_profiles.subgroup_id = subgroup.id')
                                ...
                                ->where('users.role_id','1')
                                ->where('users.status','active')
                                ->where('subgroup.id',$student_id)
                                ->get()->result();

        $count_student = $this->db->select('count(users.id) as count')
                                ->from('user_profiles')
                                ->join('users','users.id = user_profiles.user_id')
                                ->join('subgroup','user_profiles.subgroup_id = subgroup.id')
                                ->join('user_tokens','user_profiles.user_id = user_tokens.user_id')
        ...
        $grup_req_token = $request_token*$count_student[0]->count;
        ...
        $get_token = $ts->token_amount;

        // $student_token = $get_token[0]->token_amount;
        $student_token = '';
        if($get_token){
            $student_token = $ts->token_amount;    
        } else {
            $student_token = 0;
        }
        ...
        $new_token_user = $user_token-$grup_req_token;
        ...
        if($get_token){
            ...
            $update_student_token = $this->db->where('user_id', $ts->id)
                                          ->update('user_tokens', $data_student); 
        } else {
            $data_student = array(
              'user_id' => $ts->id,
        ...
        //get data for email
        $get_name = $this->user_profile_model->select('user_id, fullname')->where('user_id', $this->auth_manager->userid())->get_all();
        $get_name2 = $this->user_profile_model->select('user_id, fullname')->where('user_id', $ts->id)->get_all();
        $get_email = $this->user_model->select('id, email, role_id')->where('id', $ts->id)->get_all();
