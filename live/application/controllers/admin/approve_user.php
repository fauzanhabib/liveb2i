...
class approve_user extends MY_Site_Controller {
    ...
    public function index($type='',$page='') {
        ...
        $per_page = 6;
        $uri_segment = 5;
        // $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/manage_partner/index'), count($this->user_model->select('id, email, role_id, status')->where('status', 'disable')->order_by('id', 'desc')->get_all()), $per_page, $uri_segment);
    ...
    public function approve($id = '',$type='') {
        ...
        if (!$this->user_model->update($id, $users)) {
            ...
            redirect('admin/approve_user/'.$type);
        }
