...
//        $this->load->library('queue');
    ...
    public function index($page='') {
        ...
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/log_activity/index'), count($this->user_log_model->get_log_data()), $per_page, $uri_segment);
        $this->template->title = 'Log Activity';
        $vars = array(
            'data' => $this->user_log_model->get_log_data($per_page, $offset),
            'pagination' => $pagination
        );
        $this->template->content->view('default/contents/admin/log_activity/index',$vars);
