...
class Coach_histories extends MY_Site_Controller {
    ...
    public function index($id ='', $page=''){
        ...
        $per_page = 5;
        $uri_segment = 5;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/coach_histories/index/'.@$id), count($this->appointment_model->get_session_histories('coach_id', $id, $date_from, date('Y-m-d'))), $per_page, $uri_segment);
    ...
    //             redirect('admin/coach_histories');
    //         }elseif($session == 'class'){
    //             redirect('admin/coach_histories/class_session');
    //         }
    //     }
    ...
    public function search($session='', $id='', $page=''){
        ...
        if (!$this->common_function->run_validation($rules)) {
            $this->messages->add(validation_errors(), 'warning');
            if($session == 'one_to_one'){
                redirect('admin/coach_histories/index/'.@$id);
            }elseif($session == 'class'){
                redirect('admin/coach_histories/class_session');
            }
        }
        ...
        if($session == 'one_to_one'){
            $form_action = "search/one_to_one";
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/coach_histories/search/one_to_one/'.@$id), count($this->appointment_model->get_session_histories('coach_id', $id, $date_from, $date_to, $per_page, $offset)), $per_page, $uri_segment);
