...
class schedule extends MY_Site_Controller {
	...
	public function index()
	{
        ...
        private function schedule_block($day = '', $start_time = '', $end_time = ''){
            $offwork = $this->offwork_model->get_offwork($this->auth_manager->userid(), $s->day);
            $schedule_temp = array();
            if($offwork){
                ...
                for($i=0; $i<count($offwork);$i++){
                    ...
                    else if($i>0 && $i<(count($offwork)-1)){
                        $schedule_temp[] = array(
                            'start_time' => $offwork[$i]->end_time,
                            'end_time' => $offwork[$i+1]->start_time,
                        );
                    }
                    else if($i==(count($offwork)-1)){
                        $schedule_temp[] = array(
                            'start_time' => $offwork[$i]->end_time,
                            'end_time' => $end_time,
                        );
