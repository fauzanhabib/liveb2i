...
//                        break;
    ...
    public function update($day = '') {
        echo('<pre>');
        print_r($this->update_block($this->input->post()));
        print_r(array_search($day, $this->day_index));
        $minutes = $this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes;
        ...
        echo('schedule_gmt <br>');
        print_r($schedule_gmt0_1);
        print_r($schedule_gmt0_2);
        ...
        if (-$minutes < 0) {
            ...
            echo('Untuk schedule 1 <br>');
            print_r(array_map("unserialize", array_unique(array_map("serialize", $schedule_array1))));

            echo('Untuk schedule 2 <br>');
            print_r(array_map("unserialize", array_unique(array_map("serialize", $schedule_array2))));

            echo('untuk offwork 1 <br>');
            print_r($this->schedule_offwork_block(array_map("unserialize", array_unique(array_map("serialize", $schedule_array1)))));
            echo('untuk offwork 2 <br>');
            print_r($this->schedule_offwork_block(array_map("unserialize", array_unique(array_map("serialize", $schedule_array2)))));
        } else {
            foreach ($this->update_block($this->input->post()) as $s) {
        ...
        //$day1 = $this->schedule_offwork_block(array_map("unserialize", array_unique(array_map("serialize", $schedule_array1))));
        $this->update_schedule($day1, $this->schedule_offwork_block(array_map("unserialize", array_unique(array_map("serialize", $schedule_array1)))));
        $this->update_schedule($day2, $this->schedule_offwork_block(array_map("unserialize", array_unique(array_map("serialize", $schedule_array2)))));
        exit;
...
//                        );
//                    }
//                    else if(strtotime(-$minutes.'minutes', strtotime('00:00:00')) >= $schedule_gmt0_1[$i]['start_time'] && strtotime(-$minutes.'minutes', strtotime('00:00:00')) < strtotime($schedule_gmt0_1[$i]['end_time'])){
//                        $schedule_array1[] = array(
//                            'start_time' => date("H:i:s", strtotime(-$minutes.'minutes', strtotime('00:00:00'))),
//                            'end_time' => $schedule_gmt0_1[$i]['end_time'],
//                        );
