...
class MY_Model extends CI_Model {
    ...
    public function insert($data, $skip_validation = FALSE) {
        ...
        if ($valid) {
            $data = $this->_set_created_date($data);
    ...
    public function update($primary_value, $data, $skip_validation = FALSE) {
        ...
        $data = $this->_set_updated_date($data);
    ...
    /**
     * Paginate
     * Calculate and create pagination value
     * @param array Criteria
     * @return object Pagination
     */
    public function paged($page=1, $criteria=array(), $limit=10)
    {
        if( ! $this->pagination)
        {
    ...
    /**
     * Retrieve and generate a dropdown-tree-friendly array of the data
     * in the table based on a key and a value.
     *
     * @param string $key
     * @param string $value
     ...
     * Set Created Data
     */
    private function _set_created_date($data) {
        if($this->db->field_exists('dcrea', $this->table)) {
            $data['dcrea'] = time();
        }
        if($this->db->field_exists('dupd', $this->table)) {
            $data['dupd'] = time();
        }
        return $data;
    }
     ...
     * Set Updated Data
     */
    private function _set_updated_date($data) {
        if($this->db->field_exists('dupd', $this->table)) {
            $data['dupd'] = time();
        }
        return $data;
