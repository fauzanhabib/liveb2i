...
<table border="1">
    ...
    <tr>
        ...
        <td>
            ...
            <a href="<?php echo site_url('webex/host_meeting').'/'.@$data[0]->host_id.'/'.@$data[0]->id;?>">WebEx</a>
            <?php }else{
                echo "WebEx";
            }?>    
        </td>
...
<a onclick="webex()" id="webex_start" href="#">teset</a>
<?php }
if($data_class){?>
    <table border="1">
        <tr>
        ...
        <tr>
            ...
            <td>
                ...
                <a href="<?php echo site_url('webex/host_meeting').'/'.@$data_class[0]->host_id.'/c'.@$data_class[0]->id;?>">WebEx</a>
                <?php }else{
                    echo "WebEx";
                }?>    
            </td>
            <td><?php echo(@$data_class[0]->id); ?></td>
...
<script type="text/javascript">
    function webex(){
        $.post("<?php echo site_url('webex/get_time_remain').'/'.$this->session->userdata('meeting_identifier');?>", function( data ) {
            console.log(data);
            setTimeout(function() {
                $.post("<?php echo site_url('webex/delete_meeting').'/'.$this->session->userdata('host_id').'/'.$this->session->userdata('meeting_identifier');?>", function( data ) {
                    alert(data);
                });
            },5000);
        });
