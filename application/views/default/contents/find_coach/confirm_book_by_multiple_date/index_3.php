					...
					<div class="accordion-toggle<?=$active?>">
						...
						<span class="toggle-icon">
							<i class="icon icon-arrowdown"></i>&nbsp;&nbsp;
                                                        <i class="icon icon-close"><a href="<?php echo site_url('student/find_coaches');?>"></a></i>
						</span>
					</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		if($active > 0){
			$('.active').children('.toggle-icon').html('<i class="icon icon-arrowup"></i>&nbsp;&nbsp;<i class="icon icon-close"></i>');
		}
		
		function close_accordion() {
			$('.accordion-toggle').removeClass('active');
			$('.accordion-content').removeClass('open');
			$('.toggle-icon').html('<i class="icon icon-arrowdown"></i>&nbsp;<i class="icon icon-close"></i>');
			return false;
		}
		...
		$('.accordion-toggle').on('click', function(event){
	    	...
	    	if($(event.target).is('.active')) {
				close_accordion();
				accordionToggleIcon.html('<i class="icon icon-arrowdown"></i>&nbsp;<i class="icon icon-close"></i>');
				return false;
			}
			else {
				close_accordion();
	    		...
	    		accordionToggleIcon.html('<i class="icon icon-arrowup"></i>&nbsp;<i class="icon icon-close"></i>');
			}
