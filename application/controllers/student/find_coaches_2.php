...
class find_coaches extends MY_Site_Controller {
    ...
    public function book_single_coach($coach_id = '', $date = '', $start_time = '', $end_time = '') {
        $convert = $this->convertBookSchedule(-($this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes), $date, $start_time, $end_time);
//        echo('<pre>'); 
//        print_r($convert);
//        exit;
        $date = $convert['date'];
    ...
    public function confirm_book_by_multiple_date() {
        ...
        foreach($data as $d){
            $data_temp[] = array(
                'id' => $d->id,
                'coach_id' => $d->coach_id,
                'data' => $this->convertBookSchedule($this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes, strtotime($d->date), $d->start_time, $d->end_time),
            );
        }
//        echo('<pre>');
//        print_r($data_temp);
//        exit;
    ...
    private function email_notification_appointment($appointment_id = '') {
        ...
        $data_student = $this->convertBookSchedule(($this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes), strtotime($data_appointment->date), $data_appointment->start_time, $data_appointment->end_time);
        $data_coach = $this->convertBookSchedule(($this->identity_model->get_gmt($data_appointment->coach_id)[0]->minutes), strtotime($data_appointment->date), $data_appointment->start_time, $data_appointment->end_time);
...
//        }
        ...
        // appointment with status temporary considered available for other student but not for student who is in the appointment and 
        // appointment where the student has make an appoinment on the specific date, so there will be no the same start time and end time to be shown to the student from other coach
        $appointment_student = $this->appointment_model->select('id, date, start_time, end_time')->where('student_id', $this->auth_manager->userid())->where('status not like', 'cancel')->where('date', date("Y-m-d", $date))->where('start_time', $start_time)->where('end_time', $end_time)->get();
        // appointment coach in class
...
//        print_r(date('Y-m-d',strtotime('+ 1days'.date('Y-m-d',$date)))); 
//        exit;
