...
class request_token extends MY_Site_Controller {
    ...
    public function create() {
        ...
        // email
        $this->send_email->send_admin_request_token($get_name[0]->email, $this->input->post('token_amount'), 'requested');
        $this->send_email->send_student_supplier_request_token($adminemail[0]->email, $this->input->post('token_amount'), 'requested', $fullname);
        // =============
    ...
    public function cancel() {
        ...
        if ($id) {
            ...
            $partner_notification = array(
                'user_id' => $this->auth_manager->region_id($gpid),
                'description' => $fullname.' Clark Kent has canceled the token request.',
                'status' => 2,
                'dcrea' => time(),
                'dupd' => time(),
            );
            ...
            $this->user_notification_model->insert($partner_notification);
            // email
            $this->send_email->send_admin_request_token($get_name[0]->email, $get_token[0]->token_amount, 'canceled');
            $this->send_email->send_student_supplier_request_token($adminemail[0]->email, $get_token[0]->token_amount, 'canceled', $fullname);
