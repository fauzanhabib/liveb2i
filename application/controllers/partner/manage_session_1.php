...
class manage_session extends MY_Site_Controller {
    ...
    public function reschedule($student_id = '', $appointment_id = '') {
        ...
        if ($appointment_reschedule_data) {
            ...
            redirect('partner/manage_session/reschedule/'.$student_id.'/'.$appointment_id);
        }
        ...
        $this->template->content->view('default/contents/partner/manage_session/reschedule/schedule_detail', $vars);
    ...
    public function cancel($student_id = '', $appointment_id = '') {
        ...
        $data = array(
            ...
            'content' => 'Your appointment with student ' . $id_to_name[$student_id] . ' at ' . date("l jS \of F Y", strtotime($appointment_data->date)) . ' from ' . $appointment_data->start_time . ' until ' . $appointment_data->end_time . ' has been canceled',
        );
        ...
        $coach_notification = array(
            ...
            'description' => 'Your appointment with student ' . $id_to_name[$student_id] . ' at ' . date("l jS \of F Y", strtotime($appointment_data->date)) . ' from ' . $appointment_data->start_time . ' until ' . $appointment_data->end_time . ' has been canceled.',
            'status' => 2,
            'dcrea' => time(),
            'dupd' => time(),
        );
