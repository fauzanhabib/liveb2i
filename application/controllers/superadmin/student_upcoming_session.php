...
class Student_upcoming_session extends MY_Site_Controller {
    ...
    //     $per_page = 5;
    //     $uri_segment = 5;
    //     $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_upcoming_session/one_to_one_session/'.$student_id), count($this->appointment_model->get_appointment_for_upcoming_session('student_id', '', '', $student_id)), $per_page, $uri_segment);
    ...
    //     $this->template->content->view('default/contents/admin/student_upcoming_session/one_to_one_session', $vars);
    //     $this->template->publish();
    // }
    ...
    public function one_to_one_session($student_id='', $page='') {
        ...
        $per_page = 5;
        $uri_segment = 5;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('superadmin/student_upcoming_session/one_to_one_session/'.$student_id), count($this->appointment_model->get_appointment_for_upcoming_session('student_id', '', '', $student_id)), $per_page, $uri_segment);
        $data = $this->appointment_model->get_appointment_for_upcoming_session('student_id', '', '', $student_id, $per_page, $offset);
        if ($data) {
            foreach ($data as $d) {
    ...
    public function class_session($student_id='', $page='') {
        ...
        $per_page = 5;
        $uri_segment = 5;
        $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('superadmin/student_upcoming_session/class_session/'.@$student_id), count($this->class_member_model->get_appointment_for_upcoming_session('', '', @$student_id)), $per_page, $uri_segment);
        ...
        $this->template->content->view('default/contents/admin/student_upcoming_session/class_session', $vars);
        $this->template->publish();
    }
    ...
    //         redirect('admin/student_upcoming_session/'.$session.'_session/'.@$student_id);
    //     }
    ...
    //         $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_upcoming_session/search/one_to_one/'.@$student_id), count($this->appointment_model->get_appointment_for_upcoming_session('student_id', $date_from, $date_to, $student_id)), $per_page, $uri_segment);
    ...
    //         $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('admin/student_upcoming_session/search/class/'.@$student_id), count($this->class_member_model->get_appointment_for_upcoming_session($date_from, $date_to, $student_id)), $per_page, $uri_segment);
    ...
    //     $this->template->content->view('default/contents/admin/student_upcoming_session/' . (($session == 'one_to_one') ? 'one_to_one_session' : 'class_session'), $vars);
    //     $this->template->publish();
    // }

    public function search($session = '', $student_id='',  $page='') {
        $this->template->title = 'Coach Session';
        // echo $session." - ".$student_id." - ".$page;
        // exit();
        $adm_id = $this->auth_manager->userid();
        if($this->input->post('date_from') && $this->input->post('date_to')){
            $this->session->set_userdata('date_from', $this->input->post('date_from'));
        ...
        if (!$this->common_function->run_validation($rules)) {
            $this->messages->add(validation_errors(), 'warning');
            redirect('superadmin/student_upcoming_session/'.$session.'_session/'.@$student_id);
        }
        ...
        if ($session == 'one_to_one') {
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('superadmin/student_upcoming_session/search/one_to_one/'.@$student_id), count($this->appointment_model->get_appointment_for_upcoming_session('student_id', $date_from, $date_to, $student_id)), $per_page, $uri_segment);
            $data = $this->appointment_model->get_appointment_for_upcoming_session('student_id', $date_from, $date_to, $student_id, $per_page, $offset);
            if ($data) {
                foreach ($data as $d) {
                    $gmt_student = $this->identity_model->new_get_gmt($student_id);
            ...
            $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('superadmin/student_upcoming_session/search/class/'.@$student_id), count($this->class_member_model->get_appointment_for_upcoming_session($date_from, $date_to, $student_id)), $per_page, $uri_segment);
            $data_class = $this->class_member_model->get_appointment_for_upcoming_session($this->input->post('date_from'), $this->input->post('date_to'), $student_id, $per_page, $offset);
            if ($data_class) {
                foreach ($data_class as $data) {
                    $data_schedule = $this->schedule_function->convert_book_schedule($this->identity_model->get_gmt($this->auth_manager->userid())[0]->minutes, strtotime($data->date), $data->start_time, $data->end_time);
